...
const (
	StateIdle ecs.StateID = iota + 1 // 站立
	StateRun                         // 走动
	StateJump                        // 跳跃
...
func (n *idleNode) OnEnter(w ecs.World, e ecs.EntityID, s *ecs.State) {
	fmt.Printf("[%s] OnEnter 站立\n", n.name)
}
func (n *idleNode) OnUpdate(w ecs.World, e ecs.EntityID, s *ecs.State, dt float32) {
	// 站立行为：待机/巡逻等（示例省略）
}
func (n *idleNode) OnExit(w ecs.World, e ecs.EntityID, s *ecs.State) {
	fmt.Printf("[%s] OnExit  站立\n", n.name)
}

type runNode struct{ name string }

func (n *runNode) OnEnter(w ecs.World, e ecs.EntityID, s *ecs.State) {
	fmt.Printf("[%s] OnEnter 走动\n", n.name)
}
func (n *runNode) OnUpdate(w ecs.World, e ecs.EntityID, s *ecs.State, dt float32) {
	// 走动行为：加速度/寻路等由各自行为 System 处理，节点这里不做转换判断
}
func (n *runNode) OnExit(w ecs.World, e ecs.EntityID, s *ecs.State) {
...
type jumpNode struct {
	name       string
	groundedID ecs.ComponentID
}

func (n *jumpNode) OnEnter(w ecs.World, e ecs.EntityID, s *ecs.State) {
	fmt.Printf("[%s] OnEnter 跳跃\n", n.name)
	// 起跳离地：移除着地标记
	w.Remove(e, n.groundedID)
}
func (n *jumpNode) OnUpdate(w ecs.World, e ecs.EntityID, s *ecs.State, dt float32) {
	// 跳跃行为：抛物线等（示例省略）
}
func (n *jumpNode) OnExit(w ecs.World, e ecs.EntityID, s *ecs.State) {
...
func (n *hitNode) OnEnter(w ecs.World, e ecs.EntityID, s *ecs.State) {
	fmt.Printf("[%s] OnEnter 被打%d（硬直开始）\n", n.name, n.kind)
}
func (n *hitNode) OnUpdate(w ecs.World, e ecs.EntityID, s *ecs.State, dt float32) {
	// 硬直表现（示例省略）
}
func (n *hitNode) OnExit(w ecs.World, e ecs.EntityID, s *ecs.State) {
...
type runMoveSystem struct {
	...
	stateID ecs.ComponentID
	posID   ecs.ComponentID
	velID   ecs.ComponentID
}

func (s *runMoveSystem) Update(w ecs.World, dt float32) {
	w.Query(s.mask, func(e ecs.EntityID, q *ecs.Query) bool {
		state := q.Get(s.stateID).(*ecs.State)
...
type scriptSystem struct {
	ecs.BaseSystem
	player    ecs.EntityID
	moveCmdID ecs.ComponentID
	steps     []scriptStep
	frame     int
}

func (s *scriptSystem) Update(w ecs.World, dt float32) {
	s.frame++
...
func RunFSM() {
	...
	w := ecs.NewWorld()
	idState := ecs.RegisterComponent[ecs.State](w)
	ecs.RegisterComponent[ecs.StateContext](w)
	idMoveCmd := ecs.RegisterComponent[MoveCommand](w)
	idJumpReq := ecs.RegisterComponent[JumpRequest](w)
	idHitReq := ecs.RegisterComponent[HitRequest](w)
	idGrounded := ecs.RegisterComponent[Grounded](w)
	...
	gotHit := func(w ecs.World, e ecs.EntityID, s *ecs.State) bool { return w.Get(e, idHitReq) != nil }
	consumeHit := func(w ecs.World, e ecs.EntityID, s *ecs.State) { w.Remove(e, idHitReq) }
	hasMove := func(w ecs.World, e ecs.EntityID, s *ecs.State) bool { return w.Get(e, idMoveCmd) != nil }
	noMove := func(w ecs.World, e ecs.EntityID, s *ecs.State) bool { return w.Get(e, idMoveCmd) == nil }
	wantJump := func(w ecs.World, e ecs.EntityID, s *ecs.State) bool { return w.Get(e, idJumpReq) != nil }
	consumeJump := func(w ecs.World, e ecs.EntityID, s *ecs.State) { w.Remove(e, idJumpReq) }
	landed := func(w ecs.World, e ecs.EntityID, s *ecs.State) bool { return w.Get(e, idGrounded) != nil }
	// hitRecovered 硬直 0.1 秒（60fps 约 6 帧）后恢复。
	hitRecovered := func(w ecs.World, e ecs.EntityID, s *ecs.State) bool { return s.Duration >= 0.1 }

	// 全局状态机：节点注册表 + 转换规则拓扑（全局一份）
	m := ecs.NewStateMachine()
	...
	// 创建玩家实体：初始站立、着地
	player := w.Spawn(
		ecs.State{Current: StateIdle},
		ecs.StateContext{}, // 可选：存储目标/路点等上下文
		Grounded{},
		Pos{X: 0, Y: 0},
		Vel{Vx: 2, Vy: 0},
	)
	...
	// 系统调度：输入 → 状态机 → 行为（移动）。
	// 打乱注册顺序，Add 后 SystemManager 立即按优先级稳定排序：
	//   脚本输入(100) > 状态机(50) > 移动行为(10)
	// 业务系统内嵌 BaseSystem，通过 SetPriority 设置优先级。
	moveSys := &runMoveSystem{
		mask:    ecs.Mask(idState, idPos, idVel),
		stateID: idState,
		posID:   idPos,
		velID:   idVel,
	}
	moveSys.SetPriority(10)
	fsmSys := ecs.NewStateMachineSystem(m)
	...
	fmt.Printf("end state=%d pos=(%.2f, %.2f)\n", w.Get(player, idState).(*ecs.State).Current, pos.X, pos.Y)
	fmt.Println()
}
